package vn.lotusviet.hotelmgmt.core.service.filter;
 ...
 *
 * @param <ENTITY> the errorType of the entity which is queried.
 */
@Transactional(readOnly = true)
public abstract class FilterService<ENTITY> {
   ...
   * @param <X> The errorType of the attribute which is filtered.
   * @return a Specification
   */
  protected <X> Specification<ENTITY> buildSpecification(
      Filter<X> filter, SingularAttribute<? super ENTITY, X> field) {
   ...
   */
  protected <X> Specification<ENTITY> buildSpecification(
      Filter<X> filter, Function<Root<ENTITY>, Expression<X>> metaclassFunction) {
  ...
  protected Specification<ENTITY> buildStringSpecification(
      StringFilter filter, SingularAttribute<? super ENTITY, String> field) {
    return buildSpecification(filter, root -> root.get(field));
  }
  ...
  protected Specification<ENTITY> buildSpecification(
      StringFilter filter, Function<Root<ENTITY>, Expression<String>> metaclassFunction) {
    if (filter.getEquals() != null) {
      return equalsSpecification(metaclassFunction, filter.getEquals());
    } else if (filter.getIn() != null) {
      return valueIn(metaclassFunction, filter.getIn());
   ...
   * @param <OTHER> The errorType of the referenced entity.
   * @param <X> The errorType of the attribute which is filtered.
   * @return a Specification
   */
   ...
   * @param <OTHER> The errorType of the referenced entity.
   * @param <MISC> The errorType of the entity which is the last before the OTHER in the chain.
   * @param <X> The errorType of the attribute which is filtered.
   * @return a Specification
   */
   ...
   * greater-than-or-equal-to and null/non-null conditions are supported. Usage:
   *
   * <pre>
   *   Specification&lt;Employee&gt; specByEmployeeId = buildReferringEntitySpecification(criteria.getEmployeId(),
   * Project_.employees, Employee_.id);
  ...
  protected <OTHER, X extends Comparable<? super X>>
      Specification<ENTITY> buildReferringEntitySpecification(
  ...
  /**
   * Generic method, which based on a Root&lt;ENTITY&gt; returns an Expression which errorType is the
   * same as the given 'value' errorType.
   *
